...
var timescale = (function() {
  var data = { oid: 0, col: "#000000", nam: "Geologic Time", children: [], "lvl": 0},
  ...
  return {
    ...
    for(var i=0; i < intervals.records.length; i++) {
        var r = intervals.records[i];
        r.children = [];
        r.pid = r.pid || 0;
    ...
    var partition = d3.layout.partition()
        .sort(function(d) { return d3.ascending(d); })
    ...
    // Open to Devonian 
    timescale.goTo(interval_hash[19]);
    ...
    "labelLevels": function(d) {
      ...
      if (typeof d.parent !== 'undefined') {
        ...
        for (var i=0; i<depth;i++) {
          var parent = eval(loc).nam; // eslint-disable-line no-eval
          d3.selectAll('.abbr').filter(equals_f).attr("x", 430);
          d3.selectAll('.fullName').filter(equals_f).attr("x", 430);
          loc += ".parent";
        }
    ...
    "destroy": function (div) {
      d3.select("#" + div).html("");
      data = { oid: 0, col: "#000000", nam: "Geologic Time", children: [], "lvl": 0};
      interval_hash = { 0: data };
